...
//			$m=$this->db->get("ms_merchant");
//			$this->db->where('admin_name',$username);
//			$u=$this->db->get("ms_user");
//			if($user->num_rows()>0 || $spg->num_rows()>0 || $m->num_rows()>0 || $u->num_rows()>0) {
			if($user->num_rows()>0) {
					return false;
			} else {
					return true;
...
//		$this->db->where('created_by',$username);
//		$m=$this->db->get("ms_merchant");
//		if($user->num_rows()>0 || $m->num_rows()>0 || $u->num_rows()>0) {
		if($user->num_rows()>0) {
			return false;
		} else {
			return true;
